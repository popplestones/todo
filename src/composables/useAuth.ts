...
const verifyToken = async (): Promise<boolean> => {
  const response = await makeApiCall<void, MessageResponse | ErrorResponse>(
    "verify-token",
    "POST",
  );
...
const login = async (
...
): Promise<ApiResponse<LoginSuccessResponse | ErrorResponse> | undefined> => {
  ...
  if (response && response.status === 422)
    return response as ApiResponse<ErrorResponse>;
...
const register = async (
  ...
  ApiResponse<RegisterSuccessResponse | ErrorResponse> | undefined
> => {
...
export async function makeApiCall<TReq, TRes>(
  endpoint: string,
  method: HttpVerb = "GET",
  data?: TReq,
): Promise<ApiResponse<TRes> | undefined> {
